// KaizenMC Clone — DB compartilhado Loja + Staff
...
function defaultDB(){ return {
 settings:{
  ...
  logo:'https://kaizenmc.gg/cdn/stores/9644/theme/79ed6460-5367-467d-983c-95ed0e9e6da0.png',
  ...
  whatsapp:'',
  pixKey:'seu-pix@kaizenmc.gg', pixName:'KAIZENMC', pixCity:'BRASIL',
  autoConfirmUrl:'',
  supportEmail:'suporte@kaizenmc.gg', phone:'',
  cnpj:'© 2026 - KaizenMC. Todos os direitos reservados.',
 ...
 coupons:[{code:'KAIZEN10',percent:10}],
 ...
 load(){
  try{
   ...
   if(r){ const db=JSON.parse(r);
    ...
    db.settings.pixName=db.settings.pixName||'KAIZENMC';
...
function genPixCode(key,name,city,amount,txid){
 key=String(key||'').trim(); name=String(name||'KAIZEN').toUpperCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'').slice(0,25)||'KAIZEN';
 city=String(city||'BRASIL').toUpperCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'').slice(0,15)||'BRASIL';
 txid=String(txid||'KAIZEN').replace(/[^a-zA-Z0-9]/g,'').slice(0,20)||'KAIZEN';
