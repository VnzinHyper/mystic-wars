...
export default async function handler(req,res){
  ...
  const key=`loja:${site}`;
  const redis={
    async get(){const r=await fetch(`${URL_}/get/${encodeURIComponent(key)}`,{headers:{Authorization:`Bearer ${TOKEN}`}});if(!r.ok)throw new Error('banco indisponivel');return (await r.json()).result},
  ...
  try{
    if(req.method==='GET'){
      ...
      if(db) delete db.passHash; // nunca expor o hash
    ...
    if(req.method==='POST'){
      ...
      const raw=await redis.get();
      if(!raw) return ok(res,{ok:true,first:true,remote:false});
    ...
    if(req.method==='PUT'){
      ...
      await redis.set(JSON.stringify(current));
